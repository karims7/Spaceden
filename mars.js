...
// let MarsInsightWeather = function (
//   temperatureUnit,
//   pressureUnit,
//   windSpeedUnit
// ) {
...
//   let allTempUnit = convert().from("C").possibilities();
//   let allPressureUnit = convert().from("Pa").possibilities();
//   let allWindspeedUnit = convert().from("m/s").possibilities();
...
//   let API_CALL_LIMIT = 60 * 60; //limit api call: one hour

//   let convertData = function (data) {
//     let dataConv = JSON.parse(JSON.stringify(data)); //clone object

//     for (let i = 0; i < dataConv.sol_keys.length; i++) {
//       let sol = dataConv.sol_keys[i];
...
//   let converSolUnit = function (sol) {
//     if (that.temperatureUnit !== "C") {
//       sol.AT.av = round10(
//         convert(sol.AT.av).from("C").to(that.temperatureUnit),
//         -3
//       );
//       sol.AT.mn = round10(
//         convert(sol.AT.mn).from("C").to(that.temperatureUnit),
//         -3
//       );
//       sol.AT.mx = round10(
//         convert(sol.AT.mx).from("C").to(that.temperatureUnit),
//         -3
//       );
//     }
...
//       sol.PRE.av = round10(
//         convert(sol.PRE.av).from("Pa").to(that.pressureUnit),
//         -3
//       );
//       sol.PRE.mn = round10(
//         convert(sol.PRE.mn).from("Pa").to(that.pressureUnit),
//         -3
//       );
//       sol.PRE.mx = round10(
//         convert(sol.PRE.mx).from("Pa").to(that.pressureUnit),
//         -3
//       );
//     }
...
//       sol.HWS.av = round10(
//         convert(sol.HWS.av).from("m/s").to(that.windSpeedUnit),
//         -3
//       );
//       sol.HWS.mn = round10(
//         convert(sol.HWS.mn).from("m/s").to(that.windSpeedUnit),
//         -3
//       );
//       sol.HWS.mx = round10(
//         convert(sol.HWS.mx).from("m/s").to(that.windSpeedUnit),
//         -3
//       );
//     }
...
//     if (
//       that.lastRawData != undefined &&
//       now < that.lastRequestTime + API_CALL_LIMIT
//     ) {
...
//     return that.lastRawDataConv.sol_keys[
//       that.lastRawDataConv.sol_keys.length - 1
//     ];
//   };
